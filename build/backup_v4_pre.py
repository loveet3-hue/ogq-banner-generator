...
def make_theme(imgs, rng):
    ...
    if mode == "pastel":
        bg1 = hls(h, rng.uniform(0.86, 0.92), rng.uniform(0.45, 0.7))
        bg2 = hls((h + rng.uniform(0.03, 0.08)) % 1, rng.uniform(0.82, 0.90), rng.uniform(0.4, 0.65))
    elif mode == "peach":
    ...
    # 배경 스타일: 그라데이션 or 패턴
    bg_style = rng.choice(["grad", "grad", "grad", "blobs", "blobs", "gingham", "dots",
                           "wave_bottom", "stripes"])
    decor_pool = ["stars", "stars", "sparkle", "sparkle", "hearts", "rings", "dots_cluster", "plus"]
    bg_fx = rng.choice(["none", "none", "halo"])
    if is_dark:
        decor_cols = [(255, 255, 255), hls(h, 0.78, 0.8), hls((h + 0.5) % 1, 0.8, 0.8)]
    else:
        # 배경보다 진한 톤 1개 + 흰색 + 보색 파스텔 1개 (튀지 않게)
        decor_cols = [(255, 255, 255), hls(h, 0.62, 0.7), hls((h + 0.5) % 1, 0.72, 0.75)]
    rng.shuffle(decor_cols)
    decor_cols = decor_cols[:rng.choice([1, 2, 2])]
    return {
        "hue": h, "mode": mode, "strategy": strategy, "bg1": bg1, "bg2": bg2,
        "decor_cols": decor_cols,
        "accents": accents, "text": text_dark, "is_dark": is_dark,
        "grad_dir": rng.choice(["v", "h", "d", "radial"]),
        "bg_style": bg_style, "bg_fx": bg_fx,
        "decor": rng.sample(decor_pool, k=rng.choice([1, 1, 2])),
...
def prep_sticker(im, target_h, outline=True, outline_ratio=0.03, max_w=None, max_upscale=1.55,
                 rot=None):
...
def render_base(w, h, theme, rng=None):
    ...
    if style == "blobs":
        for _ in range(rng.randint(2, 4)):
            hh = (theme["hue"] + rng.uniform(-0.12, 0.12)) % 1.0
            col = hls(hh, 0.9 if not theme["is_dark"] else 0.35, 0.8)
            if rng.random() < 0.35:
                col = (255, 255, 255) if not theme["is_dark"] else hls(hh, 0.4, 0.7)
            rx, ry = w * rng.uniform(0.25, 0.5), h * rng.uniform(0.3, 0.6)
            draw_halo(bg, rng.uniform(0, w), rng.uniform(0, h), rx, ry, col, alpha=rng.randint(90, 160))
        return bg
    ov = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    unit = min(w, h)
    lighter = luminance(c1) < 0.6
    delta = 20 if lighter else -14
    pc = tuple(max(0, min(255, c + delta)) for c in c1[:3])
    alpha = rng.randint(30, 55)  # 은은하게
    if style == "stripes":
        sw = int(unit * rng.uniform(0.07, 0.12))
        for x in range(0, w + sw, sw * 2):
            d.rectangle([x, 0, x + sw, h], fill=(*pc, alpha))
    elif style == "diag_stripes":
        sw = int(unit * rng.uniform(0.06, 0.10))
        for x in range(-h, w + h, sw * 2):
            d.polygon([(x, 0), (x + sw, 0), (x + sw + h, h), (x + h, h)], fill=(*pc, alpha))
    elif style == "checker":
        cs = int(unit * rng.uniform(0.09, 0.14))
        for yy in range(0, h, cs):
            for xx in range(0, w, cs):
                if ((xx // cs) + (yy // cs)) % 2 == 0:
                    d.rectangle([xx, yy, xx + cs, yy + cs], fill=(*pc, alpha))
    elif style == "gingham":  # 깅엄 체크 (거북이 예시 스타일)
        cs = int(unit * rng.uniform(0.07, 0.11))
        a2 = int(alpha * 0.7)
        for x in range(0, w, cs * 2):
            d.rectangle([x, 0, x + cs, h], fill=(*pc, a2))
        for y in range(0, h, cs * 2):
            d.rectangle([0, y, w, y + cs], fill=(*pc, a2))
    elif style == "dots":
        step = int(unit * rng.uniform(0.09, 0.14))
        r = step * 0.16
        for yy in range(0, h + step, step):
            for xx in range(0, w + step, step):
                sx = xx + (step // 2 if (yy // step) % 2 else 0)
                d.ellipse([sx - r, yy - r, sx + r, yy + r], fill=(*pc, alpha + 20))
    elif style == "wave_bottom":  # 하단 부드러운 물결/구름
        amp = h * rng.uniform(0.04, 0.07)
        base_y = h * rng.uniform(0.62, 0.78)
        period = w * rng.uniform(0.35, 0.6)
        pts = [(x, base_y + amp * math.sin(2 * math.pi * x / period)) for x in range(0, w + 8, 8)]
        pts += [(w, h), (0, h)]
        col = (255, 255, 255) if lighter or luminance(c1) > 0.5 else pc
        d.polygon(pts, fill=(*col, rng.randint(90, 150)))
    elif style == "big_circles":
        for _ in range(rng.randint(2, 4)):
            r = unit * rng.uniform(0.3, 0.7)
            cx, cy = rng.uniform(0, w), rng.uniform(0, h)
            d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=(*pc, rng.randint(35, 70)))
    bg.alpha_composite(ov)
    return bg


def draw_halo(img, cx, cy, rx, ry, color, alpha=110):
    """클러스터 뒤 부드러운 원형 배경(스포트라이트)"""
    w, h = img.size
    # 알파 마스크만 블러 (색은 단색 유지 → 어두운 테두리 생기지 않음)
...
def draw_burst(img, cx, cy, r_in, r_out, color, n=14, alpha=200, rng=None):
    """히어로 뒤 집중선(버스트) 배경"""
    rng = rng or random.Random(0)
    w, h = img.size
    ov = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    ...
    for i in range(n):
        ...
        p1 = (cx + r_in * math.cos(a - spread), cy + r_in * math.sin(a - spread))
        p2 = (cx + r_in * math.cos(a + spread), cy + r_in * math.sin(a + spread))
        p3 = (cx + r_out * math.cos(a), cy + r_out * math.sin(a))
        d.polygon([p1, p2, p3], fill=(*color[:3], alpha))
    img.alpha_composite(ov)
...
def add_decor_sparse(img, theme, rng, occupied, n=None, zone=None):
    """빈 공간에만 장식 (스티커 위 X). 개수 적게, 크기 중간, 테마 색."""
    w, h = img.size
    unit = min(w, h)
    kinds = [k for k in theme["decor"] if k != "none"]
    if not kinds:
        return
    n = n if n is not None else rng.randint(3, 6)
    x0, y0, x1, y1 = zone or (w * 0.02, h * 0.04, w * 0.98, h * 0.96)
    ov = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    placed = []
    tries = 0
    while len(placed) < n and tries < 200:
        ...
        col = (*rng.choice(theme["decor_cols"])[:3], rng.randint(180, 240))
        rot = rng.uniform(0, math.pi * 2)
        if kind == "stars":
            ...
            draw_heart(d, cx, cy, r * 0.9, col, rot=rng.uniform(-0.4, 0.4))
        elif kind == "confetti":
            for _ in range(rng.randint(2, 4)):
                draw_confetti_piece(d, cx + rng.uniform(-r, r), cy + rng.uniform(-r, r),
                                    r * 0.55, (*rng.choice(theme["decor_cols"])[:3], 220),
                                    rng.uniform(0, 6.28), rng)
        elif kind == "rings":
            d.ellipse([cx - r, cy - r, cx + r, cy + r], outline=col, width=max(3, int(r * 0.25)))
        elif kind == "dots_cluster":
            for _ in range(rng.randint(3, 5)):
                rr = r * rng.uniform(0.15, 0.3)
                px, py = cx + rng.uniform(-r, r), cy + rng.uniform(-r, r)
                d.ellipse([px - rr, py - rr, px + rr, py + rr], fill=col)
        elif kind == "plus":
            lw = max(3, int(r * 0.35))
            d.line([(cx - r * 0.8, cy), (cx + r * 0.8, cy)], fill=col, width=lw)
            d.line([(cx, cy - r * 0.8), (cx, cy + r * 0.8)], fill=col, width=lw)
        elif kind == "lines":  # 짧은 강조선 3개 (만화풍)
            lw = max(3, int(r * 0.22))
            for k in (-1, 0, 1):
                a = rot + k * 0.35
                d.line([(cx + r * 0.3 * math.cos(a), cy + r * 0.3 * math.sin(a)),
...
def plan_lineup(w, h, stickers, rng, x_lo=0.0, x_hi=1.0, top=0.0, n=None,
                size=(0.72, 0.86), hero_boost=None, bottom_margin=0.03, align="bottom"):
    """
...
def plan_hero_sub(w, h, stickers, rng, x_lo=0.0, x_hi=1.0, top=0.0, side=None,
                  hero_h=(0.80, 0.92), sub_h=(0.48, 0.60), n_sub=None):
    ...
    hero = prep_sticker(hero_src, avail_h * rng.uniform(*hero_h), max_w=zone_w * 0.55, rot=tilt(rng, 0.4))
    ...
    if total > zone_w * 0.96:  # 넘치면 서브를 줄임
        sc = (zone_w * 0.96 - hero.width - gap * len(subs)) / max(1, sum(s.width for s in subs))
        sc = max(0.5, min(1.0, sc))
